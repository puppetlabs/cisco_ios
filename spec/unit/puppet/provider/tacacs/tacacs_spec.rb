...
describe Puppet::Provider::Tacacs::Tacacs do
  ...
  before(:each) do
    ...
    allow(connection).to receive(:cmd).with('show running-config | include ^ip tacacs source-interface|^tacacs-server').and_return(device_output)
    # set specific
    allow(context).to receive(:creating).with(tacacs_name).and_yield
    allow(context).to receive(:updating).with(tacacs_name).and_yield
    allow(context).to receive(:deleting).with(tacacs_name).and_yield
  ...
  test_data['default']['tests'].each do |test|
    describe test['name'] do
      ...
      it ':expectations parses to device_output' do
        changes = { tacacs_name => { is: nil, should: nil } }
        if test['previous_set'] && test['previous_set'][expectation_number]
          changes[tacacs_name][:is] = eval(test['previous_set'][expectation_number]) # rubocop:disable Security/Eval
        end
        changes[tacacs_name][:should] = eval(test['expectations'][expectation_number]) # rubocop:disable Security/Eval

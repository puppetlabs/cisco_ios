...
describe 'ios_network_dns' do
  domain_name = 'temp_hostname'

  before(:all) do
    result = run_resource('ios_network_dns')
  ...
  it 'set one way' do
    ...
    expect(result).to match(%r{ip_domain_lookup => false})
  end
  ...
  it 'set a different way' do
    ...
    expect(result).to match(%r{ip_domain_lookup => true})
  end

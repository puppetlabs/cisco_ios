...
describe 'should change an ntp_server_old' do
  before(:all) do
    # Remove if already present
    pp = <<-EOS
ntp_server_old { '1.2.3.4':
  ensure => 'absent',
}
  EOS
    make_site_pp(pp)
...
EOS
    ...
    result = run_resource('ntp_server_old', '1.2.3.4')
    expect(result).to match(%r{key.* => '42',})
    expect(result).to match(%r{ensure.* => 'present',})
  end

  it 'edit an existing ntp_server_old' do
    pp = <<-EOS
ntp_server_old { '1.2.3.4':
  ensure => 'present',
  key => 94,
...
EOS
  ...
  it 'remove an existing ntp_server_old' do
    pp = <<-EOS
ntp_server_old { '1.2.3.4':
  ensure => 'absent',
}
